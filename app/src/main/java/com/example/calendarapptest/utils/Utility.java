package com.example.calendarapptest.utils;
...
//                        if (i + 1 < s.length) prev = prev.nextnode;
    ...
    public static void getDataFromCalendarTable(Context context) {
        ...
        while (cur.moveToNext()) {
            @SuppressLint("Range") String displayName = cur.getString(cur.getColumnIndex(CalendarContract.Calendars.CALENDAR_DISPLAY_NAME));
            @SuppressLint("Range") String accountName = cur.getString(cur.getColumnIndex(CalendarContract.Calendars.ACCOUNT_NAME));
